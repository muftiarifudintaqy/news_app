...
class NewsService {
  ...
  // Method untuk mendapatkan 5 berita teratas
  static Future<List<Article>> getTop5News() async {
    try {
      final String endpoint = '/top-headlines?country=us&apiKey=$apiKey&size=5';
      final String url = _getApiUrl(endpoint);
      ...
      print('Top5 Response status: ${response.statusCode}');
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load top5 news: ${response.statusCode}');
      }
    } catch (e) {
      print('Error in getTop5News: $e');
      if (kIsWeb && e.toString().contains('CORS')) {
        throw Exception(
          'CORS Error: Please use a proxy server or enable CORS in development.',
        );
